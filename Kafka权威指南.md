...
* 如果不同步的副本可以被提升为新首领, 那么在这个副本变为不同步之后写入旧首领的消息会全部丢失, 导致数据不一致. 为什么会这样呢? 假设在`副本0`和`副本1`不可用时, 偏移量 100-200 的消息被写入`副本2`(首领). 现在`副本2`变为不可用的, 而`副本0`变为可用的. `副本0`只包含偏移量 0-100 的消息, 不包含偏移量 100-200 的消息. 如果我们允许`副本0`成为新首领, 生产者就可以继续写入数据, 消费者可以继续读取数据. 于是, 新首领就有了偏移量 100-200 的新消息. 这样, 部分消费者会读取到偏移量 100-200 的旧消息, 部分消费者会读取到偏移量 100-200 的新消息, 还有部分消费者读取的是二者的混合. 这样会导致非常不好的结果, 比如生成不准确的报表. 另外, `副本2`可能会重新变为可用, 并成为新首领的跟随者. 这个时候, 它会把比当前首领旧的消息全部删除, 而这些消息对于所有消费者来说都是不可用的.
